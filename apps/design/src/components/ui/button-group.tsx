...
const buttonGroupVariants = cva(
  "flex w-fit items-stretch overflow-hidden rounded-[6px] shadow-[var(--shadow-control)] *:focus-visible:relative *:focus-visible:z-10 has-[>[data-slot=button-group]]:items-center has-[>[data-slot=button-group]]:gap-2 has-[>[data-slot=button-group]]:overflow-visible has-[>[data-slot=button-group]]:rounded-none has-[>[data-slot=button-group]]:bg-transparent has-[>[data-slot=button-group]]:shadow-none has-[select[aria-hidden=true]:last-child]:[&>[data-slot=select-trigger]:last-of-type]:rounded-r-[6px] [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])]:border-0 [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])]:text-secondary-foreground [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])]:shadow-none [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator]):hover]:bg-accent [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator]):hover]:text-foreground [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])[aria-expanded=true]]:bg-accent [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])[aria-expanded=true]]:text-foreground [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])[data-popup-open]]:bg-accent [&>[data-slot]:not([data-slot=button-group]):not([data-slot=button-group-separator])[data-popup-open]]:text-foreground [&>[data-slot=input]]:flex-1 [&>[data-slot=input]]:py-0 [&>[data-slot=input-group]]:flex-1 [&>[data-slot=select-trigger]:not([class*='w-'])]:w-fit",
  {
    variants: {
      variant: {
        default:
        ...
        toolbar: "items-center gap-2 overflow-visible rounded-none bg-transparent shadow-none",
      },
      size: {
        default:
