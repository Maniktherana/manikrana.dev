...
function SelectValue({ className, ...props }: SelectPrimitive.Value.Props) {
  return (
    <SelectPrimitive.Value
      ...
      className={cn(
        "flex flex-1 items-center gap-1.5 px-2 text-left line-clamp-1 group-data-[size=sm]/select-trigger:px-1.5",
        className,
      )}
      {...props}
    />
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "group/select-trigger relative flex h-8 min-h-8 w-[280px] items-center justify-between gap-0 rounded-lg border border-input bg-[var(--field)] bg-clip-border p-0 font-sans text-sm leading-none font-normal tracking-normal whitespace-nowrap shadow-[var(--shadow-control)] transition-colors outline-none select-none before:pointer-events-none before:absolute before:inset-0 before:rounded-lg-inner before:shadow-[var(--shadow-highlight)] before:content-[''] focus-visible:border-ring focus-visible:shadow-[var(--shadow-control-focus)] focus-visible:before:shadow-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:shadow-[var(--shadow-control)] data-placeholder:text-muted-foreground data-[size=sm]:h-6 data-[size=sm]:min-h-6 data-[size=sm]:text-xs [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-[size=sm]:[&_svg:not([class*='size-'])]:size-3.5",
        className,
      )}
      {...props}
    >
